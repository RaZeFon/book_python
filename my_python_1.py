...
for i in range(len(array_groom)):
    print("Жених №:", i + 1)
    print(f"Имя: {array_groom[i][0]} - Качество: {array_groom[i][1]}")
    string = input("Введите что-нибудь что бы добавить к сравнению: ")
    if "" == string:
        continue
    else:
        if max_grade < array_groom[i][1]:
            max_grade = array_groom[i][1]
            grade = array_groom[i]
print(f"Выбраный жених: Имя {grade[0]} - Качество {grade[1]}")
"""     
"""
#Разборчивая невеста
import random
...
glas = "АЕЁИОУЫЭЮЯ"
alf = "БВГДЖЗЙКЛМНПРСТФХЦЧШЩАЕЁИОУЫЭЮЯ"
poloska = "#"* 50

# Выводим заставку
print(poloska)
print("              РАЗБОРЧИВАЯ НЕВЕСТА")
print(poloska)

p = 1 #номер попытки
while True: #каждая итерация цикла - попытка выбрать жениха
    
    n = random.randint(5, 30) #количество женихов
    mi = random.randint(10, 1000) #нижний уровень их качества
    ma = mi * 2 #верхний уровень их качества
    print("Итак, вы - невеста (почему бы и нет?)")
    print("Сидите вы у окошка.")
    print("А мимо окошка проходят женихи -", n, "шт.")
    print("Про каждого известно имя и показатель качества - число.")
    print("Вы должны выбрать самого лучшего.")
    print("Enter - пропустить, любое слово - выбрать.")
    input("Если готовы - нажмите Enter.")
    
    vybor = "" #выбранный жених (пока его нет)
    kach = 0 #его качество
    mx = 0 #максимальное качество в текущем наборе женихов
    
    for k in range(n): #цикл по женихам
        
        #Генерация имени жениха (слооожнооо!)
        dlina = random.randint(4, 10) #длина имени
        g = "" #сюда будем складывать буквы имени жениха
        while len(g) < dlina: 
            if len(g) > 1:
                if g[-1] in sogl and g[-2] in sogl:
                    s = glas #после двух согласных - гласная
                elif g[-1] in glas and g[-2] in glas:
                    s = sogl #после двух гласных - согласная
                else:
                    s = alf #иначе - любая буква
            else:
                s = alf #первые буквы - любые
            g += s[random.randint(0, len(s)-1)]
            #прибавили букву из выбранного ранее набора
        kg = random.randint(mi, ma) #сгенерировали качество жениха
        if kg > mx: #если жених самый качественный из просмотренных
            mx = kg
            
        # Жених готов! Запускаем.
        print("Жених №" + str(k+1) + ":", g, "-", kg)
        v = input("Ну как?")  
        if v != "": #если не Enter
            if vybor == "" or p > 3: #если жених ещё не выбран
            # или уже четвёртая попытка и мы разрешаем "перевыбирать"    
                vybor = g #запомнили выбранного жениха
                kach = kg # и его качество
            else:
                print("Нет-нет, выбор сделан, на остальных можно только смотреть:(")
    # Попытка выбора завершена
    print("Ваш жених:", vybor)
    print("Его качество:", kach)
    print("Наибольшее качество в этой когорте женихов:", mx)
    povt = input("Повторить? ДА/НЕТ: ")
    print(poloska)
    if povt.upper() == "НЕТ":
        break
    p += 1
    if p > 3:
        print("Так и быть: теперь выбор можно делать многократно!")
        print(poloska)
# Финальная реплика прогшраммы        
print("Всего доброго! Семейного счастья вам!")
"""
"""
#АРХИПЕЛАГ
import random
...
#Это наша карта. Можно сделать несколько разных,
# выбирать случайно млм предоставить выбор пользователю.
# Можно и генерировать карту - но это задача очень непростая.
#Превратили строки в списки, чтобы проще было их менять.
sea = [list("###########################"),
       list("#      #  #####   ###   # #"),
       list("#  ###   ##   ##    ##  # #"),
       list("#   ##    ##  ###      #  #"),
...
# Выведем введение
introduction = ["**************** Игра \"Архипелаг\" ***************",
                  "ЦЕЛЬ: привести кораблик (%) в нужную точку (!)\nза 60 ходов.",
                  "По пути можно собирать бонусы (+),\nкаждый даёт ещё 20 ходов.",
                  "Карты у вас нет - вы видите только то,\nчто вокруг кораблика.",
                  "Управление - клавиши U (вверх), D (вниз),\n L (влево), R (вправо), E (закончить).",
                  "************* Счастливого плавания! *************"]
for s in introduction:
    print(s)
    time.sleep(1)

#координаты кораблика    
x = 0
y = 0
# Ищем свободное место и ставим туда кораблик
while sea[y][x] != " ":
    y = random.randint(0, len(sea) - 1)
    x = random.randint(0, len(sea[0]) - 1)
sea[y][x] = "%"
...
ye = 0
while sea[ye][xe] != " ":
    ye = random.randint(0, len(sea) - 1)
    xe = random.randint(0, len(sea[0]) - 1)
sea[ye][xe] = "!"

nb = 5 #количество бонусов
xx = 0
yy = 0
while nb > 0:
    while sea[yy][xx] != " ":
        yy = random.randint(0, len(sea) - 1)
        xx = random.randint(0, len(sea[0]) - 1)
    sea[yy][xx] = "+"
    nb -= 1

# Для отладки (или если надоело проигрывать) - вывод карты    
##print()
##for s in sea:
##    print("".join(ss for ss in s)) #сборка строки из элементов списка
    
h = 60 #осталось ходов
while x != xe or y != ye: # пока кораблик не достигнет цели
    # Вывод карты - кусочек вокруг кораблика
    print("____")
    for i in range(y - 1, y + 2):
        print("".join(sea[i][j] for j in range(x - 1, x + 2)))
    # Ход пользователя
    c = input("U, D, L, R, E: ")
    h -= 1
    # Анализируем ход
    if c in "UDLRE":
        sea[y][x] = " " #если ход корректный - стёрли кораблик
        if c == "E": 
            print("Прервать? ОК.")
            break
        # Меняем координаты кораблика
        elif c == "U":
            y -= 1
        elif c == "D":
            y += 1
            ...
            x += 1
        if sea[y][x] == "#": #если вылетели на сушу
            print("Кораблекрушение:(")
            break
        elif sea[y][x] == "+":
            h += 20
            print("Бонус: 20 ходов")
        elif sea[y][x] == "!":
            print("Вы достигли цели!")
            break
        sea[y][x] = "%" #поставили кораблик на новое место
    else:
        print("Нeкорректная команда.")
    print("Осталось ходов:", h)
    if h == 0:
	...
	while True:
		...
		for i in range(num1, num2):
			if count_numbers % i != 0:
				...
				break
		if index == num2:
			print(f"Число {count_numbers} кратно {num1} и {num2}")
			break
		count_numbers += 1
"""
"""
...
# Генерировать простые числа до необходимого числа
print(primes[10000])  # Вывод 10001-го простого числ
'''
"""
